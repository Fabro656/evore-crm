...
def _migrate(conn):
    ...
    migrations = [
        ...
        # ── Backfill: eliminar asientos huerfanos de OCs/ventas borradas ──
        # Huerfano = referencia FK a registro que ya no existe (o es NULL).
        # Solo se borran los 'borrador' (o sin estado), nunca los aprobados.
        ("DELETE FROM asientos_contables WHERE subtipo IN ('orden_compra', 'anticipo_oc') AND (estado_asiento = 'borrador' OR estado_asiento IS NULL) AND (orden_compra_id IS NULL OR orden_compra_id NOT IN (SELECT id FROM ordenes_compra))"),
        ("DELETE FROM asientos_contables WHERE subtipo IN ('anticipo_venta', 'ingreso_venta') AND (estado_asiento = 'borrador' OR estado_asiento IS NULL) AND (venta_id IS NULL OR venta_id NOT IN (SELECT id FROM ventas))"),
        # Caso adicional: asientos tipo 'compra' con cuenta_debe='1405 Materias primas'
        # y sin OC vinculada (legacy antes de que existiera subtipo correcto)
        ("DELETE FROM asientos_contables WHERE tipo = 'compra' AND cuenta_debe LIKE '1405%' AND (estado_asiento = 'borrador' OR estado_asiento IS NULL) AND (orden_compra_id IS NULL OR orden_compra_id NOT IN (SELECT id FROM ordenes_compra))"),
        # Marcar asientos con pago parcial/completo como 'aprobado'
        # (estaban en 'borrador' aunque ya tuvieran cobros registrados)
        ("UPDATE asientos_contables SET estado_asiento = 'aprobado' WHERE estado_pago IN ('parcial', 'completo') AND (estado_asiento = 'borrador' OR estado_asiento IS NULL)"),
...
def _seed_puc():
    ...
    cuentas = [
        ...
        ('111010','Moneda extranjera',               5, 'debito', 'activo',      '1110', True),
        ('1115',  'Remesas en transito',             3, 'debito', 'activo',      '11', True),
        ('1120',  'Cuentas de ahorro',               3, 'debito', 'activo',      '11', False),
        ('112005','Bancos — cuentas de ahorro MN',   5, 'debito', 'activo',      '1120', True),
        ('112010','Bancos — cuentas de ahorro ME',   5, 'debito', 'activo',      '1120', True),
        ('1125',  'Fondos',                          3, 'debito', 'activo',      '11', True),
        ('13',    'Deudores',                        2, 'debito', 'activo',      '1',  False),
        ('1305',  'Clientes',                        3, 'debito', 'activo',      '13', False),
        ('130505','Nacionales',                      5, 'debito', 'activo',      '1305', True),
        ('130510','Del exterior',                    5, 'debito', 'activo',      '1305', True),
        ('130515','Deudores del sistema',            5, 'debito', 'activo',      '1305', True),
        ...
        # ═══ PACK COMPLETO Decreto 2650/1993 — Subcuentas nivel 5 ═══
        # Clase 1 — Subcuentas adicionales
        ('110515','Moneda extranjera',               5, 'debito', 'activo',      '1105', True),
        ('111505','Moneda nacional',                 5, 'debito', 'activo',      '1115', True),
        ('111510','Moneda extranjera',               5, 'debito', 'activo',      '1115', True),
        ('112505','Fondo rotatorio',                 5, 'debito', 'activo',      '1125', True),
        ('112510','Fondo de caja menor',             5, 'debito', 'activo',      '1125', True),
        ('120505','Acciones — controladas',          5, 'debito', 'activo',      '1205', True),
        ('120510','Acciones — no controladas',       5, 'debito', 'activo',      '1205', True),
        ('121505','Bonos — nacionales',              5, 'debito', 'activo',      '1215', True),
        ('121510','Bonos — extranjeros',             5, 'debito', 'activo',      '1215', True),
        ('123505','Certificados de deposito',        5, 'debito', 'activo',      '1235', True),
        ('123510','Certificados de ahorro',          5, 'debito', 'activo',      '1235', True),
        ('130520','Clientes contratos',              5, 'debito', 'activo',      '1305', True),
        ('131005','Contratos por cobrar',            5, 'debito', 'activo',      '1310', True),
        ('133005','Anticipos a proveedores',         5, 'debito', 'activo',      '1330', True),
        ('133010','Anticipos a contratistas',        5, 'debito', 'activo',      '1330', True),
        ('133015','Anticipos a trabajadores',        5, 'debito', 'activo',      '1330', True),
        ('135505','Anticipos de impuestos renta',    5, 'debito', 'activo',      '1355', True),
        ('135510','Anticipo IVA descontable',        5, 'debito', 'activo',      '1355', True),
        ('135515','Retencion en la fuente',          5, 'debito', 'activo',      '1355', True),
        ('135520','Sobrantes en liquidaciones',      5, 'debito', 'activo',      '1355', True),
        ('135525','Contribuciones',                  5, 'debito', 'activo',      '1355', True),
        ('136505','Prestamos a empleados',           5, 'debito', 'activo',      '1365', True),
        ('136510','Responsabilidades',               5, 'debito', 'activo',      '1365', True),
        ('140505','Materiales directos',             5, 'debito', 'activo',      '1405', True),
        ('140510','Materiales indirectos',           5, 'debito', 'activo',      '1405', True),
        ('143005','Productos terminados nacionales', 5, 'debito', 'activo',      '1430', True),
        ('143010','Productos terminados exportacion',5, 'debito', 'activo',      '1430', True),
        ('146505','Envases y empaques',              5, 'debito', 'activo',      '1465', True),
        ('151605','Edificios',                       5, 'debito', 'activo',      '1516', True),
        ('152005','Maquinaria y equipo — industrial',5, 'debito', 'activo',      '1520', True),
        ('152405','Muebles y enseres de oficina',    5, 'debito', 'activo',      '1524', True),
        ('152805','Equipo de procesamiento de datos',5, 'debito', 'activo',      '1528', True),
        ('152810','Equipo de telecomunicaciones',    5, 'debito', 'activo',      '1528', True),
        ('154005','Autos, camionetas y camperos',    5, 'debito', 'activo',      '1540', True),
        ('154010','Camiones, volquetas y furgones',  5, 'debito', 'activo',      '1540', True),
        ('159205','Depreciacion construcciones',     5, 'credito','activo',      '1592', True),
        ('159210','Depreciacion maquinaria',         5, 'credito','activo',      '1592', True),
        ('159215','Depreciacion equipo oficina',     5, 'credito','activo',      '1592', True),
        ('159220','Depreciacion equipo computo',     5, 'credito','activo',      '1592', True),
        ('159225','Depreciacion equipo transporte',  5, 'credito','activo',      '1592', True),
        ('170505','Intereses',                       5, 'debito', 'activo',      '1705', True),
        ('170510','Honorarios',                      5, 'debito', 'activo',      '1705', True),
        ('170515','Comisiones',                      5, 'debito', 'activo',      '1705', True),
        ('170520','Seguros y fianzas',               5, 'debito', 'activo',      '1705', True),
        ('170525','Arrendamientos',                  5, 'debito', 'activo',      '1705', True),
        ('170540','Mantenimiento',                   5, 'debito', 'activo',      '1705', True),
        ('170545','Servicios publicos',              5, 'debito', 'activo',      '1705', True),
        ('171005','Organizacion y preoperativos',    5, 'debito', 'activo',      '1710', True),
        ('171025','Mejoras a propiedades ajenas',    5, 'debito', 'activo',      '1710', True),
        ('171095','Otros cargos diferidos',          5, 'debito', 'activo',      '1710', True),
        # Clase 2 — Subcuentas adicionales
        ('210505','Sobregiros bancarios',            5, 'credito','pasivo',      '2105', True),
        ('210510','Prestamos bancarios',             5, 'credito','pasivo',      '2105', True),
        ('211005','Sobregiros',                      5, 'credito','pasivo',      '2110', True),
        ('211010','Prestamos',                       5, 'credito','pasivo',      '2110', True),
        ('220510','Casa matriz',                     5, 'credito','pasivo',      '2205', True),
        ('220515','Companias vinculadas',            5, 'credito','pasivo',      '2205', True),
        ('230505','Cuentas por pagar generales',     5, 'credito','pasivo',      '2305', True),
        ('233505','Costos y gastos por pagar',       5, 'credito','pasivo',      '2335', True),
        ('236510','Honorarios',                      5, 'credito','pasivo',      '2365', True),
        ('236515','Servicios',                       5, 'credito','pasivo',      '2365', True),
        ('236520','Arrendamientos',                  5, 'credito','pasivo',      '2365', True),
        ('236525','Rendimientos financieros',        5, 'credito','pasivo',      '2365', True),
        ('236530','Compras',                         5, 'credito','pasivo',      '2365', True),
        ('236535','Enajenacion inmuebles',           5, 'credito','pasivo',      '2365', True),
        ('236545','Loterias, rifas, apuestas',       5, 'credito','pasivo',      '2365', True),
        ('236550','Por pagos al exterior',           5, 'credito','pasivo',      '2365', True),
        ('236570','Otras retenciones y patrimonio',  5, 'credito','pasivo',      '2365', True),
        ('236725','Regimen comun',                   5, 'credito','pasivo',      '2367', True),
        ('236730','Regimen simplificado',            5, 'credito','pasivo',      '2367', True),
        ('236805','Industria y comercio',            5, 'credito','pasivo',      '2368', True),
        ('237005','Aportes EPS',                     5, 'credito','pasivo',      '2370', True),
        ('237006','Aportes pension',                 5, 'credito','pasivo',      '2370', True),
        ('237010','Aportes ARL',                     5, 'credito','pasivo',      '2370', True),
        ('237025','Aportes caja compensacion',       5, 'credito','pasivo',      '2370', True),
        ('237030','Aportes SENA',                    5, 'credito','pasivo',      '2370', True),
        ('237035','Aportes ICBF',                    5, 'credito','pasivo',      '2370', True),
        ('240405','Vigencia fiscal corriente',       5, 'credito','pasivo',      '2404', True),
        ('240805','IVA generado',                    5, 'credito','pasivo',      '2408', True),
        ('241205','Impuesto industria y comercio',   3, 'credito','pasivo',      '24',   True),
        ('2412',  'Impuesto industria y comercio',   3, 'credito','pasivo',      '24',   True),
        ('2420',  'De vehiculos',                    3, 'credito','pasivo',      '24',   True),
        ('2435',  'A las ventas retenido',           3, 'credito','pasivo',      '24',   True),
        ('2436',  'A la propiedad raiz',             3, 'credito','pasivo',      '24',   True),
        ('2495',  'Otros',                           3, 'credito','pasivo',      '24',   True),
        ('250505','Salarios por pagar',              5, 'credito','pasivo',      '2505', True),
        ('251005','Ley laboral anterior',            5, 'credito','pasivo',      '2510', True),
        ('251010','Ley 50 de 1990 y posteriores',    5, 'credito','pasivo',      '2510', True),
        ('252005','Prima de servicios',              5, 'credito','pasivo',      '2520', True),
        ('252505','Vacaciones',                      5, 'credito','pasivo',      '2525', True),
        ('260505','Para obligaciones fiscales',      5, 'credito','pasivo',      '2605', True),
        ('260510','Para obligaciones laborales',     5, 'credito','pasivo',      '2605', True),
        ('260515','Para garantias',                  5, 'credito','pasivo',      '2605', True),
        ('270505','Ingresos recibidos por anticipado',5,'credito','pasivo',      '2705', True),
        ('280505','De clientes',                     5, 'credito','pasivo',      '2805', True),
        ('280510','Sobre contratos',                 5, 'credito','pasivo',      '2805', True),
        # Clase 3 — Subcuentas adicionales
        ('310505','Capital autorizado',              5, 'credito','patrimonio',  '3105', True),
        ('310510','Capital por suscribir (DB)',      5, 'debito', 'patrimonio',  '3105', True),
        ('310515','Capital suscrito por cobrar (DB)',5, 'debito', 'patrimonio',  '3105', True),
        ('320505','Prima en colocacion de acciones', 5, 'credito','patrimonio',  '3205', True),
        ('3210',  'Donaciones',                      3, 'credito','patrimonio',  '32',   True),
        ('3215',  'Credito mercantil',               3, 'credito','patrimonio',  '32',   True),
        ('330505','Reserva legal',                   5, 'credito','patrimonio',  '3305', True),
        ('331505','Reserva para readquisicion acciones',5,'credito','patrimonio','3315',True),
        ('340505','Ajustes por inflacion',           5, 'credito','patrimonio',  '3405', True),
        ('360505','Utilidad del ejercicio',          5, 'credito','patrimonio',  '3605', True),
        ('361005','Perdida del ejercicio',           5, 'debito', 'patrimonio',  '3610', True),
        ('370505','Utilidades acumuladas',           5, 'credito','patrimonio',  '3705', True),
        ('371005','Perdidas acumuladas',             5, 'debito', 'patrimonio',  '3710', True),
        # Clase 4 — Subcuentas adicionales
        ('413505','Ventas — mostrador',              5, 'credito','ingreso',     '4135', True),
        ('413510','Ventas — domicilio',              5, 'credito','ingreso',     '4135', True),
        ('414005','Servicios profesionales',         5, 'credito','ingreso',     '4140', True),
        ('414010','Servicios tecnicos',              5, 'credito','ingreso',     '4140', True),
        ('417005','Devoluciones en ventas',          5, 'debito', 'ingreso',     '4170', True),
        ('417010','Descuentos comerciales en ventas',5, 'debito', 'ingreso',     '4170', True),
        ('421005','Intereses ganados',               5, 'credito','ingreso',     '4210', True),
        ('421010','Dividendos y participaciones',    5, 'credito','ingreso',     '4210', True),
        ('422005','Comerciales arrendamientos',      5, 'credito','ingreso',     '4220', True),
        ('422505','Comisiones sobre ventas',         5, 'credito','ingreso',     '4225', True),
        ('425005','Recuperacion provisiones',        5, 'credito','ingreso',     '4250', True),
        ('429505','Diversos — aprovechamientos',     5, 'credito','ingreso',     '4295', True),
        ('429510','Diversos — indemnizaciones',      5, 'credito','ingreso',     '4295', True),
        # Clase 5 — Subcuentas adicionales de personal
        ('510515','Horas extras y recargos',         5, 'debito', 'gasto',       '5105', True),
        ('510518','Comisiones',                      5, 'debito', 'gasto',       '5105', True),
        ('510521','Viaticos',                        5, 'debito', 'gasto',       '5105', True),
        ('510524','Incapacidades',                   5, 'debito', 'gasto',       '5105', True),
        ('510542','Dotacion y suministros',          5, 'debito', 'gasto',       '5105', True),
        ('510548','Bonificaciones',                  5, 'debito', 'gasto',       '5105', True),
        ('510551','Dominicales y festivos',          5, 'debito', 'gasto',       '5105', True),
        ('511005','Honorarios — juntas',             5, 'debito', 'gasto',       '5110', True),
        ('511010','Honorarios — revisoria fiscal',   5, 'debito', 'gasto',       '5110', True),
        ('511015','Honorarios — auditoria externa',  5, 'debito', 'gasto',       '5110', True),
        ('511020','Honorarios — asesoria tecnica',   5, 'debito', 'gasto',       '5110', True),
        ('511025','Honorarios — asesoria juridica',  5, 'debito', 'gasto',       '5110', True),
        ('511505','Impuestos municipales',           5, 'debito', 'gasto',       '5115', True),
        ('511510','Industria y comercio',            5, 'debito', 'gasto',       '5115', True),
        ('511515','Vehicular',                       5, 'debito', 'gasto',       '5115', True),
        ('511520','A la propiedad raiz',             5, 'debito', 'gasto',       '5115', True),
        ('512005','Arrendamientos — terrenos',       5, 'debito', 'gasto',       '5120', True),
        ('512010','Arrendamientos — construcciones', 5, 'debito', 'gasto',       '5120', True),
        ('512015','Arrendamientos — maquinaria',     5, 'debito', 'gasto',       '5120', True),
        ('512020','Arrendamientos — equipo oficina', 5, 'debito', 'gasto',       '5120', True),
        ('512025','Arrendamientos — equipo computo', 5, 'debito', 'gasto',       '5120', True),
        ('513505','Aseo y vigilancia',               5, 'debito', 'gasto',       '5135', True),
        ('513510','Asistencia tecnica',              5, 'debito', 'gasto',       '5135', True),
        ('513515','Publicidad y propaganda',         5, 'debito', 'gasto',       '5135', True),
        ('513520','Transportes fletes y acarreos',   5, 'debito', 'gasto',       '5135', True),
        ('513540','Correos y portes',                5, 'debito', 'gasto',       '5135', True),
        ('513550','Gastos bancarios',                5, 'debito', 'gasto',       '5135', True),
        ('514505','Mantenimiento construcciones',    5, 'debito', 'gasto',       '5145', True),
        ('514510','Mantenimiento maquinaria',        5, 'debito', 'gasto',       '5145', True),
        ('514515','Mantenimiento equipo oficina',    5, 'debito', 'gasto',       '5145', True),
        ('514520','Mantenimiento equipo computo',    5, 'debito', 'gasto',       '5145', True),
        ('515505','Alojamiento y manutencion',       5, 'debito', 'gasto',       '5155', True),
        ('515510','Pasajes aereos',                  5, 'debito', 'gasto',       '5155', True),
        ('515515','Pasajes terrestres',              5, 'debito', 'gasto',       '5155', True),
        ('519505','Comisiones',                      5, 'debito', 'gasto',       '5195', True),
        ('519510','Libros, suscripciones, revistas', 5, 'debito', 'gasto',       '5195', True),
        ('519515','Mercadeo y publicidad',           5, 'debito', 'gasto',       '5195', True),
        ('519520','Utiles, papeleria y fotocopias',  5, 'debito', 'gasto',       '5195', True),
        ('519525','Combustibles y lubricantes',      5, 'debito', 'gasto',       '5195', True),
        ('519530','Envases y empaques',              5, 'debito', 'gasto',       '5195', True),
        ('519535','Elementos de aseo y cafeteria',   5, 'debito', 'gasto',       '5195', True),
        ('530505','Intereses',                       5, 'debito', 'gasto',       '5305', True),
        ('530510','Comisiones',                      5, 'debito', 'gasto',       '5305', True),
        ('530515','Diferencia en cambio',            5, 'debito', 'gasto',       '5305', True),
        ('530520','Descuentos comerciales condicion.',5,'debito', 'gasto',       '5305', True),
        ('531005','Perdida en venta de inversiones', 5, 'debito', 'gasto',       '5310', True),
        ('531010','Perdida en venta de PPE',         5, 'debito', 'gasto',       '5310', True),
        ('540505','Impuesto de renta',               5, 'debito', 'gasto',       '5405', True),
        # Clase 6 — Costos de venta por actividad
        ('6205',  'Costo de ventas — por contrato de serv',3,'debito','costo_venta','62', True),
        ('620505','Costo servicios profesionales',   5, 'debito', 'costo_venta', '6205', True),
        ('613505','Costo mercancia vendida',         5, 'debito', 'costo_venta', '6135', True),
        # Clase 7 — Costos de produccion
        ('710505','Materia prima consumida',         5, 'debito', 'costo_produccion', '7105', True),
        ('720505','Mano de obra directa — salarios', 5, 'debito', 'costo_produccion', '7205', True),
        ('720510','MOD — prestaciones sociales',     5, 'debito', 'costo_produccion', '7205', True),
        ('730505','Costos indirectos — depreciacion',5, 'debito', 'costo_produccion', '7305', True),
        ('730510','Costos indirectos — arrendamientos',5,'debito','costo_produccion','7305',True),
        ('730515','Costos indirectos — servicios publicos',5,'debito','costo_produccion','7305',True),
        ('730520','Costos indirectos — mantenimiento',5,'debito','costo_produccion','7305', True),
        ('730525','Costos indirectos — seguros',     5, 'debito', 'costo_produccion', '7305', True),
        # Clase 8/9 — Cuentas de orden adicionales
        ('8110',  'Litigios y demandas',             3, 'debito', 'orden_deudora', '81', True),
        ('8115',  'Promesas de compraventa',         3, 'debito', 'orden_deudora', '81', True),
        ('8120',  'Bienes recibidos en custodia',    3, 'debito', 'orden_deudora', '81', True),
        ('8195',  'Otras responsabilidades',         3, 'debito', 'orden_deudora', '81', True),
        ('82',    'Deudoras fiscales',               2, 'debito', 'orden_deudora', '8',  False),
        ('8205',  'Deudoras de control — fiscales',  3, 'debito', 'orden_deudora', '82', True),
        ('8315',  'Deudoras de control',             3, 'debito', 'orden_deudora', '83', True),
        ('9110',  'Bienes recibidos en garantia',    3, 'credito','orden_acreedora', '91', True),
        ('9115',  'Bienes recibidos en comodato',    3, 'credito','orden_acreedora', '91', True),
        ('9120',  'Litigios y demandas',             3, 'credito','orden_acreedora', '91', True),
        ('9125',  'Promesas de compraventa',         3, 'credito','orden_acreedora', '91', True),
        ('9195',  'Otras responsabilidades',         3, 'credito','orden_acreedora', '91', True),
        ('9205',  'Acreedoras fiscales',             3, 'credito','orden_acreedora', '92', True),
        ('9395',  'Otras acreedoras de control',     3, 'credito','orden_acreedora', '93', True),
    ]
